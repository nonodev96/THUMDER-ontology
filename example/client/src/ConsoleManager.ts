...
export class ConsoleManager {
  ...
  private async emit() {
    socket.emit(
      "tests",
      { client: "Hello world console manager" },
      (response: any) => {
  ...
  private async requestModifyFile() {
    const modifyFile = new ModifyFile(
      "path/to/folder",
      "tests/",
      "example_00.s"
    );
    ...
    return this.coreAgents.addTask(
  ...
  private async requestCreateFile() {
    ...
    return this.coreAgents.addTask(
      new Task_CreateFile_RequestInitiator("Make-File", message)
    );
  }

  private async requestCreateFolder() {
    const createFolder = new CreateFolder("path/to/folder", "newFolder/");
    ...
    return this.coreAgents.addTask(
      new Task_CreateFolder_RequestInitiator("Make-Folder", message)
    );
  }
  ...
  private async cfpNewSimulation() {
    ...
    return this.coreAgents.addTask(
      new Task_ContractNetInitiator("CFP-Simulation", cfp)
    );
  }
}
